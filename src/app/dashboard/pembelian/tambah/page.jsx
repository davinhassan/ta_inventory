...
export default function TambahPOPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (dataStok.data && Array.isArray(dataStok.data))
          setStokBarang(dataStok.data);
  ...
  const handleSupplierChange = (e) => {
    ...
    if (cart.length > 0 && newId !== supplierId) {
      if (
        confirm(
          "Perhatian: Mengganti supplier akan MENGHAPUS daftar barang yang sudah dipilih (karena beda supplier). Lanjutkan?"
        )
      ) {
  ...
  // --- 5. Cart Actions ---
  const handleAddItem = () => {
    if (!selectedBarangId || qty <= 0)
      return alert("Pilih barang dan jumlah valid!");

    const existingItem = cart.find((item) => item.id === selectedBarangId);
    if (existingItem) return alert("Barang ini sudah ada di daftar.");

    const barangInfo = stokBarang.find(
      (b) => b.id === parseInt(selectedBarangId)
    );
    ...
    <AuthGuard allowedRoles={["ADMIN", "MANAJER", "PEMILIK", "STAFF"]}>
      <div className="p-4 md:p-8 max-w-4xl mx-auto pb-24">
        ...
        <h1 className="text-xl md:text-2xl font-bold text-foreground mb-6">
          Buat Request Barang Masuk (PO)
        </h1>

        <div className="bg-card p-4 md:p-6 rounded-xl border border-border shadow-sm space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <select
                className="w-full bg-background border border-input text-foreground p-3 rounded-lg focus:ring-2 focus:ring-blue-500 text-sm outline-none"
                value={supplierId}
              ...
              <p className="text-xs text-muted-foreground mt-2">
                *Barang yang muncul akan disesuaikan dengan supplier yang
                dipilih.
              </p>
            </div>
            ...
            <div>
              ...
              {!previewUrl ? (
                <div className="relative group">
                  ...
                  />
                  <div className="bg-background border border-dashed border-input rounded-lg p-3 flex items-center gap-3 text-muted-foreground group-hover:border-blue-500 transition">
                ...
                <div className="relative h-32 w-full rounded-lg overflow-hidden border border-border group">
                  <Image
                    src={previewUrl}
                    alt="Preview"
                    fill
                  ...
                  <button
                    onClick={clearFile}
                    className="absolute top-2 right-2 bg-red-600 text-white p-1 rounded-full hover:bg-red-700 transition z-20 shadow-md"
                  >
                    <X size={16} />
                  </button>
          ...
          <div className="bg-secondary/30 p-4 rounded-lg border border-border">
            <div className="flex justify-between items-center mb-4">
              ...
              {supplierId && (
                <span className="text-xs bg-blue-100 text-blue-700 dark:bg-blue-900/50 dark:text-blue-300 px-2 py-1 rounded border border-blue-200 dark:border-blue-800">
                  Supplier Terpilih:{" "}
                  {suppliers.find((s) => s.id == supplierId)?.namaSupplier}
                </span>
              )}
            </div>
            ...
            <div className="flex flex-col md:flex-row gap-4 items-end">
              <div className="w-full md:flex-1">
                <label className="text-xs text-muted-foreground mb-1 block">
                  Barang (Hanya dari Supplier Terpilih)
                </label>
                <select
                  className="w-full bg-background border border-input text-foreground p-2.5 rounded text-sm disabled:opacity-50 disabled:cursor-not-allowed outline-none focus:ring-2 focus:ring-blue-500"
              ...
              <div className="w-full md:w-24">
                ...
                <input
                  type="number"
                  min="1"
                  className="w-full bg-background border border-input text-foreground p-2.5 rounded text-sm outline-none focus:ring-2 focus:ring-blue-500"
                  value={qty}
                ...
                {cart.length === 0 ? (
                  ...
                  cart.map((item, idx) => (
                    ...
                    >
                      ...
                      <td className="p-3 text-center text-sm font-mono text-foreground">
                        {item.qty}
                      </td>
                      <td className="p-3 text-right pr-4">
                        <button
                          onClick={() => handleRemoveItem(item.id)}
                          className="text-red-400 hover:text-red-500 hover:bg-muted p-1.5 rounded transition"
                        >
                          <Trash size={16} />
                        </button>
                      </td>
